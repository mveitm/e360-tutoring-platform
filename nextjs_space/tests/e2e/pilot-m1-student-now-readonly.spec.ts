...
test('PILOT_M1_001 can view completed and pending M1 pilot state without mutation', async ({ page }) => {
  ...
  console.log(`SAFE_E2E_AUTH_REQUEST_COUNT: ${authRequestRecorder.count()}`)
  console.log(`SAFE_E2E_AUTH_REQUEST_SEQUENCE: ${authRequestRecorder.summary()}`)
  console.log(`SAFE_E2E_CREDENTIALS_CALLBACK_OBSERVED: ${authRequestRecorder.observed('callback-credentials') ? 'yes' : 'no'}`)
  console.log(`SAFE_E2E_SESSION_REQUEST_OBSERVED_AFTER_SUBMIT: ${authRequestRecorder.observed('session') ? 'yes' : 'no'}`)
  console.log(`SAFE_E2E_CSRF_REQUEST_OBSERVED_AFTER_SUBMIT: ${authRequestRecorder.observed('csrf') ? 'yes' : 'no'}`)
  console.log(`SAFE_E2E_PROVIDERS_REQUEST_OBSERVED_AFTER_SUBMIT: ${authRequestRecorder.observed('providers') ? 'yes' : 'no'}`)
